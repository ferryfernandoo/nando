...
class ImageGenerationService {
   ...
   * - Editing mode (with referenceImage): uses qwen-image-edit-max
   */
  static async generateImage(prompt, size = '1024x1024', sessionId = null, model = null, referenceImage = null, abortSignal = null) {
    try {
      ...
      if (!finalModel) {
        finalModel = isEditMode ? 'qwen-image-edit-max' : 'imagen-4-fast';
      }
      ...
      if (isEditMode) {
        console.log('🔴🔴🔴 [IMAGE_GEN_DEBUG] Reference image provided - Edit mode enabled (qwen-image-edit-max)');
      } else {
        console.log('🔴🔴🔴 [IMAGE_GEN_DEBUG] No reference image - Generation mode (imagen-4-fast)');
      }
      ...
      if (!response.ok) {
        ...
        console.error('🔴🔴🔴 [IMAGE_GEN_DEBUG] API error:', error);
        throw new Error(`Image ${isEditMode ? 'editing' : 'generation'} failed: ${response.status}`);
      }
  ...
  /**
   * Generate AI reasoning for image generation process using Orion AI
  ...
  static async generateAIReasoning(originalPrompt, englishPrompt) {
    try {
      ...
      const reasoningPrompt = `You are Orion AI, an advanced image generation reasoning engine. 
